...
if __name__ == "__main__":
    ...
    plt.hist(source_pixels, bins=50, alpha=0.6, density=True, label="Source")
    plt.hist(target_pixels, bins=50, alpha=0.6, density=True, label="Target")
    plt.xlabel("Pixel intensity")
    plt.ylabel("Density")
    plt.title("Pixel intensity distribution: Source vs Target")
    plt.legend()
    plt.tight_layout()
    plt.savefig(FIG_DIR / "dataset_histograms.png", dpi=300)
    plt.close()

    means = [source_pixels.mean(), target_pixels.mean()]
    stds  = [source_pixels.std(),  target_pixels.std()]
    ...
    plt.title("Per-domain mean and standard deviation")
    plt.legend()
    plt.tight_layout()
    plt.savefig(FIG_DIR / "dataset_statistics.png", dpi=300)
    plt.close()
